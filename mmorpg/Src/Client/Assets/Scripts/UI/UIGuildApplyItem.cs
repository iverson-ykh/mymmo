...
public class UIGuildApplyItem : ListView.ListViewItem
{
    ...
    {
        MessageBox.Show(string.Format("要通过[{0}]的加入审批吗", this.Info.Name), "审批申请", MessageBoxType.Confirm, "确认", "取消").OnYes = () =>
        {
    ...
    {
        MessageBox.Show(string.Format("要拒绝[{0}]的加入审批吗", this.Info.Name), "审批申请", MessageBoxType.Confirm, "确认", "取消").OnYes = () =>
        {
