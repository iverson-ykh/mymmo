...
using UnityEngine.UI;
    ...
    public void OnItemSelected(ListView.ListViewItem item) {
        ...
        this.descript.text = DataManager.Instance.Items[this.selectedItem.item.id].Description;
        //this.descript.text = this.selectedItem.item.Define.Descript;
    }
    ...
    void InitItems() {
        foreach (var kv in ItemManager.Instance.Items) {
            if (kv.Value.Define.Type==ItemType.Ride&&(
                ...
                GameObject go = Instantiate(itemPrefab,this.Root.transform);
