...
public class TeleporterObject : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (playerController!=null&&playerController.isActiveAndEnabled) {
            ...
            if (td==null) {
                Debug.LogErrorFormat("TeleporterObject:Character [{0}] enter the Teleporter [{1}],but TeleporterDefine is not existed",playerController.character.Info.Name,this.ID);
                return;
            }
            Debug.LogFormat("what up{0}",this.ID);
            Debug.LogFormat("TeleporterObject:Character [{0}] enter the Teleporter [{1}]:[{2}]", playerController.character.Info.Name, td.ID,td.Name);
            if (td.LinkTo>0) {
